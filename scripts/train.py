...
from src.utils import (create_data_loaders, get_class_names,
...
def train_epoch(
    ...
    train_bar = tqdm(
        train_loader, desc=f"Epoch {epoch+1}/{config['training']['epochs']}"
    )
...
def validate_epoch(
    model: nn.Module, val_loader, device: torch.device, config: dict
) -> tuple:
    """
    검증 에포크
...
def main():
    ...
    parser.add_argument(
        "--experiment_name",
        type=str,
        default="attention_mil_training",
        help="MLflow 실험 이름",
    )
    ...
    with mlflow.start_run():
        ...
        for epoch in range(config["training"]["epochs"]):
            ...
            # 검증
            val_loss, val_acc = validate_epoch(model, val_loader, device, config)
        ...
        # MLflow에 최종 결과 로깅
        mlflow.log_metrics(
            {"best_val_acc": best_val_acc, "training_time": training_time}
        )
