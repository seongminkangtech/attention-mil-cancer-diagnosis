...
class APITester:
    ...
    def test_prediction_endpoint(self, image_path: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if image_path and os.path.exists(image_path):
                ...
                response = requests.post(f"{self.base_url}/predict", json=dummy_data, timeout=30)
    ...
    def load_test(self, endpoint: str, num_requests: int = 100, concurrent: int = 10) -> Dict[str, Any]:
        """부하 테스트 수행"""
        results = []
    ...
    def comprehensive_test(self) -> Dict[str, Any]:
        ...
        prediction_load_result = self.load_test("predict", num_requests=20, concurrent=5)
...
class APIMonitor:
    ...
    def continuous_monitoring(self, interval: int = 60, duration: int = 3600):
        """지속적인 API 모니터링"""
        logger.info(f"🔄 API 지속 모니터링 시작 (간격: {interval}초, 지속: {duration}초)")
        ...
        while time.time() - start_time < duration:
            try:
                ...
                # 결과 출력
                if 'error' in health_result:
                    logger.error(f"❌ {result['timestamp']} - API 오류: {health_result['error']}")
                else:
                    logger.info(f"✅ {result['timestamp']} - API 정상 (응답시간: {health_result['response_time']:.3f}초)")
    ...
    def _print_monitoring_summary(self, results: List[Dict[str, Any]]):
        ...
        response_times = [r['health'].get('response_time', 0) for r in results if 'error' not in r['health']]
...
def main():
    ...
    parser.add_argument("--concurrent", type=int, default=10, help="부하 테스트 동시 요청 수")
    parser.add_argument("--monitor-interval", type=int, default=60, help="모니터링 간격 (초)")
    parser.add_argument("--monitor-duration", type=int, default=3600, help="모니터링 지속 시간 (초)")
