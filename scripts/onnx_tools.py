...
class ONNXConverter:
    ...
    def convert_to_onnx(self, output_path: str, input_shape: tuple = (1, 3, 224, 224)) -> bool:
...
class ONNXValidator:
    ...
    def validate_model(self, model_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, input_info in enumerate(model.graph.input):
                ...
                model_info[f'input_{i}_type'] = str(input_info.type.tensor_type.elem_type)
            
            for i, output_info in enumerate(model.graph.output):
                model_info[f'output_{i}_shape'] = str(output_info.type.tensor_type.shape)
                model_info[f'output_{i}_type'] = str(output_info.type.tensor_type.elem_type)
    ...
    def test_inference(self, model_path: str, input_shape: tuple = (1, 3, 224, 224)) -> bool:
        """ONNX 모델 추론 테스트"""
        try:
            # ONNX Runtime 세션 생성
...
def main():
    ...
    if args.action == "convert":
        ...
        output_path = args.output_path or args.model_path.replace('.onnx', '_optimized.onnx')
        optimizer.optimize_model(args.model_path, output_path)
    
    elif args.action == "quantize":
        # ONNX 모델 양자화
        optimizer = ONNXOptimizer(config)
        output_path = args.output_path or args.model_path.replace('.onnx', '_quantized.onnx')
