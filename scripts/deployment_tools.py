...
class DockerManager:
    ...
    def build_image(
        self, tag: str = None, dockerfile: str = None, build_args: Dict[str, str] = None
    ) -> bool:
        """Docker 이미지 빌드"""
        try:
...
class KubernetesManager:
    ...
    def deploy_to_namespace(
        self, namespace: str = "dev", config_files: List[str] = None
    ) -> bool:
        ...
        try:
            ...
            if namespace_file.exists():
                subprocess.run(
                    ["kubectl", "apply", "-f", str(namespace_file)], check=True
                )
            ...
            for config_file in config_files:
                ...
                if file_path.exists():
                    subprocess.run(
                        ["kubectl", "apply", "-f", str(file_path)], check=True
                    )
...
class ModelValidator:
    ...
    def validate_model_performance(
        self, model_path: str, test_data_path: str = None
    ) -> Dict[str, Any]:
        """모델 성능 검증"""
        try:
    ...
    def compare_models(self, model_paths: List[str]) -> Dict[str, Any]:
        ...
        if comparison_results:
            sorted_models = sorted(
                comparison_results.items(), key=lambda x: x[1]["avg_inference_time_ms"]
            )
...
class DeploymentOrchestrator:
    ...
    def full_deployment_pipeline(
        self, model_path: str, image_tag: str = None, namespace: str = "dev"
    ) -> bool:
        """전체 배포 파이프라인 실행"""
        try:
...
def main():
    ...
    if args.action == "docker-build":
        ...
        if not args.deployment or not args.replicas:
            ...
            return
        orchestrator.k8s_manager.scale_deployment(
        ...
        if not args.model_path:
            ...
            return
        result = orchestrator.model_validator.validate_model_performance(
            args.model_path
        )
