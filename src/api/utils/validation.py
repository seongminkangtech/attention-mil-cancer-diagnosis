...
def validate_config(config: Dict[str, Any]) -> bool:
    ...
    """
    required_keys = ["model", "data", "training", "paths", "hardware"]
...
def validate_image_input(
    image_files: List[str], max_files: int = 50, allowed_extensions: List[str] = None
) -> bool:
    """
    이미지 입력 검증
    ...
    if len(image_files) > max_files:
        raise ValueError(
            f"이미지 파일 수가 최대 허용치를 초과했습니다. (최대: {max_files})"
        )
...
def validate_prediction_request(request_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for field in required_fields:
        if field not in request_data:
            raise HTTPException(
                status_code=400, detail=f"필수 필드가 누락되었습니다: {field}"
            )
    ...
    if not isinstance(image_paths, list):
        raise HTTPException(
            status_code=400, detail="image_paths는 리스트 형태여야 합니다."
        )
...
def validate_model_config(config_path: str) -> Dict[str, Any]:
    ...
    try:
        if not os.path.exists(config_path):
            raise HTTPException(
                status_code=500, detail=f"설정 파일을 찾을 수 없습니다: {config_path}"
            )
        ...
        raise HTTPException(status_code=500, detail=f"설정 파일 파싱 오류: {str(e)}")
    except ValueError as e:
        raise HTTPException(status_code=500, detail=f"설정 파일 검증 오류: {str(e)}")
