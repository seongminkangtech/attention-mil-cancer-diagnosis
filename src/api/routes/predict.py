...
def load_onnx_model(config: Dict[str, Any]) -> ONNXModel:
    ...
    try:
        # ONNX 모델 경로 설정
        onnx_model_path = config.get("deployment", {}).get(
            "onnx_model_path", "models/attention_mil.onnx"
        )
...
def preprocess_images_for_prediction(
    image_paths: List[str], config: Dict[str, Any]
) -> np.ndarray:
    """
    추론을 위한 이미지 전처리
    ...
    image_array = np.expand_dims(
        image_array, axis=0
    )  # (1, image_count, 3, img_size, img_size)
        ...
        if _model is None:
            if not load_model():
                return create_error_response(
                    message="모델을 로드할 수 없습니다.", error_code="MODEL_LOAD_ERROR"
                )
...
async def get_model_info() -> Dict[str, Any]:
    ...
    if _model is None:
        return create_error_response(
            message="모델이 로드되지 않았습니다.", error_code="MODEL_NOT_LOADED"
        )
    ...
    return create_response(success=True, message="모델 정보 조회 완료", data=model_info)
    ...
    try:
        ...
        if success:
            return create_response(
                ...
                data={"model_loaded": True, "model_type": "ONNX"},
            )
        else:
            return create_error_response(
                message="모델 재로드에 실패했습니다.", error_code="MODEL_RELOAD_ERROR"
            )
