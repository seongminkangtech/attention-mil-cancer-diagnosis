...
class ONNXModel:
    ...
    def predict(
        ...
        try:
            ...
            if not isinstance(input_data, np.ndarray):
                raise ValueError(
                    f"입력 데이터는 numpy 배열이어야 합니다. 현재 타입: {type(input_data)}"
                )

            # 입력 데이터 형태 검증
            if input_data.ndim != 5:
                raise ValueError(
                    f"입력 데이터는 5차원이어야 합니다. 현재 차원: {input_data.ndim}"
                )
    ...
    def preprocess_input(
        self, images: List[np.ndarray], target_shape: Tuple[int, ...]
    ) -> np.ndarray:
        """
        입력 이미지 전처리
    ...
    def postprocess_output(
        self, prediction: np.ndarray, class_names: List[str]
    ) -> Dict[str, Any]:
        """
        출력 후처리
