...
def load_and_preprocess_images(data_path: str,
                             csv_file: str,
                             label_file: str,
                             image_count: int = 50,
                             img_size: int = 256) -> Tuple[List[str], torch.Tensor, torch.Tensor]:
    """
    의료 영상 데이터 로드 및 전처리
    ...
    for i in tqdm(range(len(image_list)), desc="이미지 처리"):
        ...
        if len(image_file_list) > image_count:
            # 랜덤 샘플링
            image_index = torch.randint(low=0, high=len(image_file_list) - image_count, size=(1,))
            start_idx = image_index.item()
            
            for count in range(image_count):
                image = Image.open(image_file_list[start_idx + count]).resize((img_size, img_size))
            ...
            for j in range(image_count - len(image_file_list)):
                image = Image.open(image_file_list[j % len(image_file_list)]).resize((img_size, img_size))
                image = 1 - tf(image)
...
def create_data_loaders(id_list: List[str],
