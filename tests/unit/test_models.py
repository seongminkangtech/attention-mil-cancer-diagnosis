...
class TestAttentionMILModel:
    ...
    def test_attention_mil_attention_weights(self):
        ...
        assert torch.allclose(torch.sum(attention_weights, dim=1), torch.ones(batch_size), atol=1e-6)
    ...
    def test_attention_mil_invalid_input(self):
        ...
        invalid_input = torch.randn(2, 3, 256, 256)  # (B, C, H, W) - num_tiles 차원 누락
    ...
    def test_onnx_model_predict(self):
        ...
        with patch('os.path.exists', return_value=True):
            with patch('onnxruntime.InferenceSession') as mock_session:
                ...
                mock_session_instance.run.return_value = [np.random.rand(1, 3), np.random.rand(1, 10)]
    ...
    def test_onnx_model_input_shape_validation(self):
        ...
        with patch('os.path.exists', return_value=True):
            with patch('onnxruntime.InferenceSession') as mock_session:
                ...
                # 잘못된 형태의 입력
                invalid_input = np.random.rand(3, 256, 256).astype(np.float32)  # 차원 부족
