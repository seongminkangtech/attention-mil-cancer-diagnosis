...
class AttentionConfig:
    ...
    def __post_init__(self):
        ...
        if self.dropout_rate < 0 or self.dropout_rate > 1:
            raise ValueError(
                f"드롭아웃 비율은 0-1 사이여야 합니다: {self.dropout_rate}"
            )
...
class LoggingConfig:
    ...
    def __post_init__(self):
        """환경 변수에서 설정 로드"""
        self.tensorboard = (
            os.getenv("TENSORBOARD", str(self.tensorboard)).lower() == "true"
        )
...
class DeploymentConfig:
    ...
    def __post_init__(self):
        ...
        if self.optimization_level < 0 or self.optimization_level > 3:
            raise ValueError(
                f"최적화 레벨은 0-3 사이여야 합니다: {self.optimization_level}"
            )
    ...
    def from_yaml(cls, yaml_path: str) -> "AttentionMILConfig":
        ...
        if "model" in config_dict:
            ...
            if "feature_extractor" in model_config:
                ...
                config.feature_extractor.model_name = fe_config.get(
                    "model_name", config.feature_extractor.model_name
                )
                config.feature_extractor.pretrained = fe_config.get(
                    "pretrained", config.feature_extractor.pretrained
                )
            if "attention" in model_config:
                att_config = model_config["attention"]
                config.attention.hidden_dim = att_config.get(
                    "hidden_dim", config.attention.hidden_dim
                )
                config.attention.dropout_rate = att_config.get(
        ...
        if "data" in config_dict:
            ...
            config.data.image_count = data_config.get(
                "image_count", config.data.image_count
            )
            config.data.img_size = data_config.get("img_size", config.data.img_size)
            config.data.batch_size = data_config.get(
                "batch_size", config.data.batch_size
            )
            config.data.train_ratio = data_config.get(
        ...
        if "training" in config_dict:
            ...
            config.training.epochs = training_config.get(
                "epochs", config.training.epochs
            )
            config.training.learning_rate = training_config.get(
                "learning_rate", config.training.learning_rate
            )
            config.training.optimizer = training_config.get(
                "optimizer", config.training.optimizer
            )
            config.training.scheduler = training_config.get(
                "scheduler", config.training.scheduler
            )
            config.training.step_size = training_config.get(
        ...
        if "hardware" in config_dict:
            ...
            config.hardware.device = hardware_config.get(
                "device", config.hardware.device
            )
            config.hardware.num_workers = hardware_config.get(
        ...
        if "logging" in config_dict:
            ...
            config.logging.log_interval = logging_config.get(
                "log_interval", config.logging.log_interval
            )
            config.logging.save_interval = logging_config.get(
                "save_interval", config.logging.save_interval
            )
            config.logging.tensorboard = logging_config.get(
        ...
        if "evaluation" in config_dict:
            ...
            config.evaluation.metrics = evaluation_config.get(
                "metrics", config.evaluation.metrics
            )
            config.evaluation.save_predictions = evaluation_config.get(
                "save_predictions", config.evaluation.save_predictions
            )
            config.evaluation.confusion_matrix = evaluation_config.get(
                "confusion_matrix", config.evaluation.confusion_matrix
            )

        # 배포 설정
        if "deployment" in config_dict:
            deployment_config = config_dict["deployment"]
            config.deployment.model_type = deployment_config.get(
                "model_type", config.deployment.model_type
            )
            config.deployment.onnx_model_path = deployment_config.get(
                "onnx_model_path", config.deployment.onnx_model_path
            )
            config.deployment.providers = deployment_config.get(
                "providers", config.deployment.providers
            )
            config.deployment.optimization_level = deployment_config.get(
                "optimization_level", config.deployment.optimization_level
            )
            config.deployment.enable_profiling = deployment_config.get(
                "enable_profiling", config.deployment.enable_profiling
            )
